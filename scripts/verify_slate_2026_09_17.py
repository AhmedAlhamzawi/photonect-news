#!/usr/bin/env python3
"""Post-copywriter verification for the 2026-09-17 slate.
...
after publication:
  ...
  4. exactly 2 statPops per brief, each matchWord matching EXACTLY ONE timing word.
     ...
     silently dropped. That is how 2026-09-10 slug B shipped with pops=1: the
  ...
  7. NUMBER DRIFT: the SET of distinct digit-runs in the shipped files is
     compared against the authored originals (imported from author_2026_09_10,
...
for slug in sorted(p.name for p in POSTS.glob(f"{D}-*") if p.is_dir()):
    ...
    # 2026-09-11: the Friday dollar pulse is one outlet's own bourse-close series
    # (Shafaq 27 Aug vs 10 Sep). No second outlet carried the 10 Sep close; the
    # only candidates found were stale (May/June). Ship single-source, flagged —
    # never pad the list with an outlet the reel doesn't use.
    min_src = 2
    if not isinstance(src, list) or len(src) < min_src or not all(
            isinstance(x, dict) and {"name", "domain"} <= set(x) for x in src):
        fails.append(f"{slug}: sources is not a list of >={min_src} {{name,domain}} dicts")
    elif len(src) < 2:
        warns.append(f"{slug}: single-source ({src[0]['name']}) — flag in DELIVERY")
    ...
    if brief:
        ...
        if "تسعمئة وأربعة وستين" in vo:
            fails.append(f"{slug}: voText speaks the outlet 964 as a number")
        if re.search(r"\d", vo):

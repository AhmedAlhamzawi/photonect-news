#!/usr/bin/env bash
# render-apr20-rehunts.sh — re-render + re-QA the 7 April 20 slugs that
# failed brightness QA on the first pass (after hero floor was raised 62→80).
set -u
...
failed=(); qa_failed=()
log="/tmp/photonect-apr20/render-rehunts.log"
: > "$log"

qa_reel() {
  local slug="$1"
  local mp4="data/posts/$slug/newsreel_v3.mp4"
  local tmpdir="/tmp/photonect-apr20/qa-rehunt-$slug"
  mkdir -p "$tmpdir"
...
if fails:
    print(f"QA2-FAIL {slug} {','.join(fails)}")
    sys.exit(1)
else:
    print(f"QA2-PASS {slug} hero={res['hero']['L']} b1={res['beat1']['L']} b2={res['beat2']['L']} b3={res['beat3']['L']}")
PY
}

echo "=== RE-RENDER START $(date) ==="
for slug in "${SLUGS[@]}"; do
  ...
  if bash "$HERE/render-reel.sh" "$slug" v3 >>"$log" 2>&1; then
    ...
    echo "RERENDER-OK $slug ${dur}s ${bytes}B"
    ok=$((ok+1))
...
echo ""
echo "=== RE-RENDER ALL DONE $(date) ==="
echo "rerender ok=$ok fail=$fail qa-fail=$qa_fail"
[ ${#failed[@]} -gt 0 ]   && echo "RERENDER FAILED: ${failed[*]}"
[ ${#qa_failed[@]} -gt 0 ] && echo "QA2 STILL FAILED: ${qa_failed[*]}"
exit $(( fail + qa_fail ))

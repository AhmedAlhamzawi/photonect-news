#!/usr/bin/env bash
# render-apr20-rehunts3.sh — 3rd re-render pass for the 5 April 20 slugs
# that passed luminance QA but had off-topic hero imagery. These now use
# props.breaking.heroQueries curated seeds that feed into synth_queries()
# before ATLAS fallback — a brand-new architectural affordance landed
# 2026-04-20 03:47 alongside this script.
set -u
...
failed=(); qa_failed=()
log="/tmp/photonect-apr20/render-rehunts3.log"
: > "$log"

qa_reel() {
  local slug="$1"
  local mp4="data/posts/$slug/newsreel_v3.mp4"
  local tmpdir="/tmp/photonect-apr20/qa-rehunt3-$slug"
  mkdir -p "$tmpdir"
...
if fails:
    print(f"QA4-FAIL {slug} {','.join(fails)}")
    sys.exit(1)
else:
    print(f"QA4-PASS {slug} hero={res['hero']['L']} b1={res['beat1']['L']} b2={res['beat2']['L']} b3={res['beat3']['L']}")
PY
}

echo "=== RE-RENDER3 START $(date) ==="
for slug in "${SLUGS[@]}"; do
  ...
  if bash "$HERE/render-reel.sh" "$slug" v3 >>"$log" 2>&1; then
    ...
    echo "RERENDER3-OK $slug ${dur}s ${bytes}B"
    ok=$((ok+1))
    ...
    dur=$(( $(date +%s) - start ))
    echo "RERENDER3-FAIL $slug ${dur}s"
...
echo ""
echo "=== RE-RENDER3 ALL DONE $(date) ==="
echo "rerender3 ok=$ok fail=$fail qa-fail=$qa_fail"
[ ${#failed[@]} -gt 0 ]   && echo "RERENDER3 FAILED: ${failed[*]}"
[ ${#qa_failed[@]} -gt 0 ] && echo "QA4 STILL FAILED: ${qa_failed[*]}"
exit $(( fail + qa_fail ))
